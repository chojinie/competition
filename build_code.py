...
# data load
train_data = pd.read_csv('/workspace/jin_space/compet/train.csv')
test_data = pd.read_csv('/workspace/jin_space/compet/test.csv')

# data preprocessing

# 1. missing value handling : ['전용면적', '해당층', '총층', '방수', '욕실수', '총주차대수']
# 별도 ipynb에서 확인 함. KNN 활용하여 결측치 채우기. # 모델은 뒤에서...
knn_filling_data_cols = ['전용면적', '해당층', '총층', '방수', '욕실수', '총주차대수']

# 2. categorical data encoding (학습 데이터)
one_hot_cols = ['매물확인방식', '방향', '주차가능여부', '중개사무소', '제공플랫폼'] # 학습 데이터셋에서 중개사무소 279 종류.

# 3.중개사무소별 허위매물 비율 계산
smoothing = 10  # 스무딩 하이퍼파라미터
avg_false_rate = train_data['허위매물여부'].mean()  # 전체 평균 허위매물 비율

# 중개사무소별 허위매물 비율 계산
office_stats = train_data.groupby('중개사무소')['허위매물여부'].agg(['mean', 'count'])
office_stats['스무딩허위매물비율'] = (office_stats['mean'] * office_stats['count'] + avg_false_rate * smoothing) / (office_stats['count'] + smoothing)

#  원본 데이터에 스무딩된 허위매물 비율 추가
train_data = train_data.merge(office_stats[['스무딩허위매물비율']], how='left', left_on='중개사무소', right_index=True)
test_data = test_data.merge(office_stats[['스무딩허위매물비율']], how='left', left_on='중개사무소', right_index=True)

# 테스트 데이터에 없는 중개사무소는 전체 평균 허위매물 비율로 채움
test_data['스무딩허위매물비율'] = test_data['스무딩허위매물비율'].fillna(avg_false_rate)

# 3. 기존 '중개사무소' 컬럼 제거 (One-Hot Encoding 제거)
one_hot_cols.remove('중개사무소')
train_data.drop(columns=['중개사무소'], inplace=True)
test_data.drop(columns=['중개사무소'], inplace=True)


# One-Hot Encoding
one_hot_encoder = OneHotEncoder(handle_unknown='ignore')    # test시 훈련에서 못봤던 범주의 경우 0으로 강제하는 옵션 발동하는 instance생성.
train_encoded = one_hot_encoder.fit_transform(train_data[one_hot_cols]).toarray()
train_encoded_df = pd.DataFrame(train_encoded, columns=one_hot_encoder.get_feature_names_out(one_hot_cols), index=train_data.index) # categorical data rebuilding
...
def xgb_cv(n_estimators, max_depth, learning_rate, subsample, colsample_bytree, reg_alpha, reg_lambda, min_child_weight):
    ...
    for train_index, val_index in kf.split(X):
        ...
        # DMatrix 변환
        dtrain = xgb.DMatrix(X_tr_preprocessed, label=y_tr)
        ...
        y_val_pred_binary = (y_val_pred > 0.5).astype(int)  # 0.5 기준으로 이진 분류
        
        # 정확도 계산
        accuracy_scores.append(accuracy_score(y_val, y_val_pred_binary))
    
    mean_accuracy = np.mean(accuracy_scores)
    print(f"Fold Accuracy: {mean_accuracy:.4f}")
